...
{   
    ...
    //public E_BugType _bugType=E_BugType.Default;


    public bool isDied;//呱：标记虫子死掉没有
    ...
    public Sprite insectImage
    {
        get
        {
            if (_insectImage == null)
            {
                if (insectId != 0)
                {
                    return Id_To_Insect_Dic.IdToInsectDic[insectId].insectImage;
                }
            }
            return _insectImage;
        }
        set { _insectImage = value; }
    }
    private Sprite _insectImage;
